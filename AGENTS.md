...
- **Vault PIN** — setup enforces a 6–12 character PIN; use e.g. `123456` when manually testing vault setup.
- **Product direction** — client-only (no kbox server), not “data never leaves the device”. Source of truth: `docs/requirements/10-direction.md`. Do not reintroduce OneDrive or “No cloud account” positioning. Google Drive auto-sync stays off unless the user enables it.
